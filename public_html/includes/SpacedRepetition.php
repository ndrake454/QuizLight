...
class SpacedRepetition {
    ...
    public function initializeCard($userId, $questionId) {
        try {
            ...
            $stmt = $this->pdo->prepare("
                INSERT INTO sr_cards (user_id, question_id, ease_factor, review_interval, repetitions, next_review)
                VALUES (?, ?, 2.5, 1, 0, ?)
            ");
    ...
    public function processReview($userId, $questionId, $quality) {
        try {
            ...
            $stmt = $this->pdo->prepare("SELECT id, ease_factor, review_interval, repetitions FROM sr_cards WHERE user_id = ? AND question_id = ?");
            $stmt->execute([$userId, $questionId]);
            ...
            if ($stmt->rowCount() === 0) {
                ...
                $stmt = $this->pdo->prepare("SELECT id, ease_factor, review_interval, repetitions FROM sr_cards WHERE id = ?");
                $stmt->execute([$cardId]);
            }
            ...
            $reviewInterval = $card['review_interval'];
            ...
            if ($quality < 3) {
                ...
                if ($repetitions == 1) {
                    ...
                    $reviewInterval = round($reviewInterval * $easeFactor);
                }
            }
            ...
            $nextReview = date('Y-m-d H:i:s', strtotime("+{$reviewInterval} days"));
            
            // Update card in database
            $stmt = $this->pdo->prepare("
                UPDATE sr_cards 
                SET ease_factor = ?, review_interval = ?, repetitions = ?, next_review = ?, last_review = NOW()
                WHERE id = ?
            ");
            $stmt->execute([$easeFactor, $reviewInterval, $repetitions, $nextReview, $cardId]);
            
            // Log the review
            $stmt = $this->pdo->prepare("
                INSERT INTO sr_review_log (card_id, user_id, question_id, quality, ease_factor, review_interval)
                VALUES (?, ?, ?, ?, ?, ?)
            ");
            $stmt->execute([$cardId, $userId, $questionId, $quality, $easeFactor, $reviewInterval]);
    ...
    public function getUserStats($userId) {
        try {
            ...
            $stmt = $this->pdo->prepare("
                SELECT COUNT(*) FROM sr_cards WHERE user_id = ? AND review_interval > 30
            ");
            $stmt->execute([$userId]);
