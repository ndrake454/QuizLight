<?php
/**
 * Live Quiz Answer Handler
 * 
 * Processes answer submissions from participants in live quizzes
 */
...
try {
    ...
    if ($timedOut) {
        ...
        $acceptableAnswers = $stmt->fetchAll(PDO::FETCH_COLUMN);
        ...
        foreach ($acceptableAnswers as $answer) {
            if (normalizeText($answer) === $normalizedUserAnswer) {
                $isCorrect = true;
                break;
            }
        }
        
        // If no exact match, try fuzzy matching for short answers
        if (!$isCorrect && strlen($normalizedUserAnswer) <= 30) {
            foreach ($acceptableAnswers as $answer) {
                ...
                $maxLength = max(strlen($normalizedUserAnswer), strlen($normalizedAnswer));
                $threshold = min(0.8, 1 - (2 / $maxLength)); // Adaptive threshold
                $maxAllowedDistance = floor($maxLength * (1 - $threshold));
                
                $distance = levenshtein($normalizedUserAnswer, $normalizedAnswer);
        ...
        // Calculate points based on correctness and time taken (same formula as multiple choice)
        if ($isCorrect) {
            // Base points for correct answer
            $basePoints = 1000;
